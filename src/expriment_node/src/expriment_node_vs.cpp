...
int trigger(int n_cur,double ped1_x,double ped1_y){
    if(n_cur%2==1)  
    {
        return 0;
    }
    double del2=(check_ped1_begin[0]-ped1_x)*(check_ped1_begin[0]-ped1_x)
    +(check_ped1_begin[1]-ped1_y)*(check_ped1_begin[1]-ped1_y);
    std::cout <<"now ped1_x: "<< check_ped1_begin[0]<<" ped1_y: "<<check_ped1_begin[1]<<std::endl;
        ...
        if(exper_node.n_cur<=exper_node.N_expre){  //i < 200
            if(exper_node.n_cur==0){ //start
               ...
               exper_node.collision=0;
                ...
                if (trigger(exper_node.n_cur,0.624831,-6.39258)){
...
{
    check_ped1_begin[0]=msg->data[3*0];
    check_ped1_begin[1]=msg->data[3*0+1];
...
int main(int argc, char** argv)
{
    ...
    nh.param<int>("/expriment_node_vs/obstacle_num", obstacle_num,0);
    nh.param<double>("/expriment_node_vs/robot_r", car_r,0);
    nh.param<double>("/expriment_node_vs/obstacle_radius", ob_r,0);
    nh.param<double>("/expriment_node_vs/goal_dis_exper", goal_dis,0);
    nh.param<double>("/expriment_node_vs/time_max_exper", time_max,0);
