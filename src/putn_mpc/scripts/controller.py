...
class Controller():
    ...
    def get_current_state(self, event):
        try:
            ...
            self.curr_state[4] =cur_time_tf.to_sec()
            c = Float32MultiArray()
            c.data = [self.curr_state[0], self.curr_state[1], self.curr_state[2],
                    (self.curr_state[3]+np.pi) % (2*np.pi)-np.pi, roll, pitch,cur_time_tf.to_sec()]

            self.pub2.publish(c)
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException,TransformException):
            pass
